...
// The VersionInfoFunc type is an adapter to allow the use of ordinary
// function as VersionInfo mutator.
type VersionInfoFunc func(context.Context, *ent.VersionInfoMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f VersionInfoFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.VersionInfoMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.VersionInfoMutation", m)
}
